...
public class BancoHSBCNegocio extends BancoNegocio {
	
	private static Logger logger = new Logger(BancoHSBCNegocio.class);

	public static EnumSet<EspecieTitulo> ESPECIE_TITULOS = EnumSet.of(
			EspecieTitulo.DM,
			EspecieTitulo.DS,
			EspecieTitulo.DR,
			EspecieTitulo.LC,
	...
	private Set<EspecieTitulo> especieTitulos = new LinkedHashSet<EspecieTitulo>();
	
	public BancoHSBCNegocio() {
		banco = Banco.BANCO_HSBC;
		for (EspecieTitulo especie : ESPECIE_TITULOS) {
			especieTitulos.add(especie);
			//especie.getValor();
		}
	}
	...
	public String retornarCabecalhoArquivoRemesaCNAB240(Cedente cedente) {
		...
		cabecalhoArquivo.versaoLayoutArquivo = "010";
	...
	public String retornarDetalheSegmentoQRemessa(int numeroRegistro, Boleto boleto) {
		...
		temp = TextoUtil.retiraSimbolos(boleto.getSacado().getEndereco().getCep());
		if (temp != null)
			registroDetalhe.cep = Long.parseLong(temp);
